...
const ChartCard = ({ title, subtitle, children, isEmpty }) => (
    // [MODIFICADO]: Estilo Glassmorphism Oscuro (Translúcido, bordes blancos suaves, sombras intensas)
    <div className="bg-[#0b2241]/80 backdrop-blur-xl rounded-[24px] border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] p-4 sm:p-6 flex flex-col h-full transition-all duration-300 hover:shadow-[0_10px_35px_rgba(34,211,238,0.1)] hover:border-cyan-500/30 relative group overflow-hidden">
        {/* Resplandor (Glow) que aparece al pasar el cursor */}
...
// --- GRÁFICOS LOCALES (CONTRASTES OSCUROS) ---
const LocalStackedBar = React.memo(({ data, keys, isCurrency }) => (
    <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ left: -15, bottom: 40, right: 10, top: 40 }}>
            {/* Malla muy sutil para no interferir */}
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(255,255,255,0.06)" />
            
            {/* Textos de los ejes claros */}
            <XAxis dataKey="name" tick={{fontSize: 9, fontWeight: 700, fill: '#e2e8f0'}} axisLine={false} tickLine={false} angle={-45} textAnchor="end" height={60} dy={10} interval={0} />
            <YAxis tick={{fontSize: 9, fill: '#94a3b8', fontWeight: 600}} axisLine={false} tickLine={false} tickFormatter={(v) => isCurrency ? `$${(v/1000000).toFixed(0)}M` : v} />
            ...
            <Tooltip 
                ...
                formatter={(val) => isCurrency ? `$${val.toLocaleString()}` : val.toLocaleString()}
            />
            ...
            {/* CORRECCIÓN: El comentario ahora está adentro de <Bar> y no afuera flotando */}
            {keys.map((k) => (
                <Bar key={k} dataKey={k} stackId="a" fill={getSafeColor(k)} barSize={38} stroke="#1e293b" strokeWidth={1.5}>
                    {/* El stroke se mantiene oscuro para delinear la barra frente al texto de la etiqueta */}
...
// --- SUNBURST (AJUSTE MILIMÉTRICO DE TEXTO Y ESPACIO) ---
const InteractiveSunburst = React.memo(({ level1, level2, focusedNode, onNodeClick }) => {
    ...
    const renderSmartLabel = useCallback((props) => {
        ...
        // Porcentajes verdaderos matemáticos
        ...
        // --- SOLUCIÓN DEL DESBORDAMIENTO: Escala estricta al arco ---
        // Calculamos cuánto espacio en píxeles tiene exactamente esta tajada
        const arcSpace = radius * (sliceAngle * RADIAN);
        
        let fontSizeText = isLevel2 ? 10 : 11;
        let fontSizePct = isLevel2 ? 9 : 10;
        
        // Si la tajada mide menos de ~35px de ancho, forzamos a que la letra se encoja 
        // para que jamás se salga de las líneas negras
        if (arcSpace < 35) {
    ...
    const CustomTooltip = ({ active, payload }) => {
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-slate-900/95 backdrop-blur-md border border-white/10 p-3 rounded-xl shadow-2xl text-white z-50">
...
export default function Cartera({ data, selectedFilters }) {
    ...
    // --- PROCESAMIENTO E INFLADO CONTROLADO ---
    const buildSunburstData = useCallback((list, level1Key, level2Key, valKey) => {
        ...
        for (const [_, node] of rootMap) {
            ...
            for (const [childName, childValue] of node.children.entries()) {
                ...
                // --- EL PISO VISUAL (Garantiza visibilidad sin deformar el resto) ---
                let visualSize = childValue;
                if (childValue > 0) {
                    // Si el valor es minúsculo, lo subimos a un tamaño equivalente al 3.5% del pastel global
                    // para que exista espacio físico para renderizarlo
                ...
                childrenOfNode.push({
                    name: childName, 
                    value: childValue, // Dato Verdadero
                    visualValue: visualSize, // Tamaño Pincelado
            ...
            level1.push({ 
                name: node.name, 
                value: node.value, 
                visualValue: visualTotalForParent, // La suma exacta de sus hijos
                globalTotal 
            });
        }
