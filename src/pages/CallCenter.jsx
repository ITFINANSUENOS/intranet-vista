...
import {
    Layers, Database, Search, ChevronDown, ChevronUp,
    Columns, ChevronLeft, ChevronRight, Phone, MessageSquare, Bell
} from 'lucide-react';
...
import {
    ...
    useCallCenterTabs,
    useCallCenterCharts,
    useTablaRemotaCallCenter,
    useTablaRemotaCallCenterGestion
...
const SummaryCard = ({ title, value, color = '#ffffff' }) => (
    <div className="bg-[#0b2241]/80 backdrop-blur-xl rounded-[16px] border border-white/10 shadow-[0_8px_30px_rgba(0,0,0,0.3)] p-4 sm:p-5 flex flex-col justify-center transition-all duration-300 hover:bg-white/5 hover:border-white/20">
        ...
        <p className="text-xl sm:text-[22px] font-black font-mono tracking-tight" style={{ color }}>
            {value}
        </p>
    </div>
);
            ...
            <div className="bg-slate-900/95 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-2xl z-50 pointer-events-none">
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center gap-6">
                        ...
                        <span className="font-semibold text-white text-[12px] text-right">{data.count}</span>
                    </div>
                    <div className="flex justify-between items-center gap-6">
                        <span className="text-[11px] text-slate-400 uppercase tracking-wider">Porcentaje:</span>
                        <span className="font-bold text-white text-[13px]">{data.percentage}%</span>
                    </div>
                </div>
            </div>
        );
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE COMPARTIDO: SELECTOR DE COLUMNAS (ColumnMenu)
// ─────────────────────────────────────────────────────────────────────────────

const ColumnMenu = React.memo(({ allColumns, visibleColumns, onToggleColumn, isMenuOpen, onMenuToggle }) => {
    ...
    const filteredColumns = useMemo(
        () => allColumns.filter(col => col.label.toLowerCase().includes(columnSearch.toLowerCase())),
                ...
                <div className="absolute right-0 mt-2 w-72 bg-[#0b2241] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl">
                    ...
                    <div className="max-h-60 overflow-y-auto p-2 custom-scrollbar">
                        ...
                        {filteredColumns.length === 0 && (
                            <div className="p-4 text-center text-slate-500 text-sm">No se encontraron columnas</div>
                        )}
                    </div>
                </div>
            )}
...
// ─────────────────────────────────────────────────────────────────────────────
// TOOLBAR: DETALLE DE OPERACIÓN CALL CENTER (filtros: Rodamiento, Gestión, Pago, Novedades)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// TOOLBAR: CALL CENTER – GESTIÓN Y NOVEDADES
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE DE TABLA GENÉRICO CON PAGINACIÓN
// ─────────────────────────────────────────────────────────────────────────────
                ...
                <table className="w-full text-left border-collapse min-w-max">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {data.map((row, rowIndex) => (
                            <tr key={rowIndex} className="hover:bg-white/[0.02] transition-colors duration-200 group">
            ...
            {pagination.total_pages > 1 && (
                <div className="bg-[#061428] border-t border-white/10 p-4 flex items-center justify-between">
                    <div className="text-sm text-slate-400">
                        Mostrando página{' '}
                        <span className="font-medium text-white">{pagination.current}</span> de{' '}
                        <span className="font-medium text-white">{pagination.total_pages}</span>
...
// ─────────────────────────────────────────────────────────────────────────────
// TABLA COMPUESTA: DETALLE DE OPERACIÓN CALL CENTER
// ─────────────────────────────────────────────────────────────────────────────

const TablaRemotaCallCenter = ({ titulo, origen, apiClient, jobId, selectedFilters }) => {
    const {
        data, allColumns, visibleColumns, loading, pagination,
        localFilters, columnsToRender, filtrosExport,
        handleFilterChange, handlePageChange, toggleColumn
    } = useTablaRemotaCallCenter(apiClient, jobId, origen, selectedFilters);
...
// ─────────────────────────────────────────────────────────────────────────────
// TABLA COMPUESTA: CALL CENTER – GESTIÓN Y NOVEDADES
// ─────────────────────────────────────────────────────────────────────────────

const TablaRemotaCallCenterGestion = ({ titulo, origen, apiClient, jobId, selectedFilters }) => {
    const {
        dataCG, allColumnsCG, visibleColumnsCG, loadingCG, paginationCG,
        localFiltersCG, columnsToRenderCG, filtrosExportCG,
        handleFilterChangeCG, handlePageChangeCG, toggleColumnCG
    } = useTablaRemotaCallCenterGestion(apiClient, jobId, origen, selectedFilters);
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTE PRINCIPAL: CallCenter
// ─────────────────────────────────────────────────────────────────────────────

export default function CallCenter({ data, selectedFilters, apiClient, jobId }) {

    const { activeTab, setActiveTab } = useCallCenterTabs('llamadas');
    const { chartData, pieData, tableTotals } = useCallCenterCharts(data, selectedFilters);
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* GRÁFICO 1: BARRAS — Rendimiento Call Center */}
                <ChartCard
                ...
                >
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} layout="vertical" margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
                            <CartesianGrid strokeDasharray="3 3" horizontal vertical stroke="rgba(255,255,255,0.06)" />
                            <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11, fontWeight: 600 }} tickFormatter={(v) => `${v}%`} />
                ...
                {/* GRÁFICO 2: CIRCULAR — Rodamientos en Call Centers */}
                <ChartCard
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-[12px] sm:text-[13px]">
                            {chartData.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="p-8 text-center text-slate-500 font-bold uppercase tracking-widest">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                <SummaryCard title="Meta Total"     value={formatCurrency(tableTotals.meta)}                                   color="#ffffff" />
                <SummaryCard title="Recaudo Total"  value={formatCurrency(tableTotals.recaudo)}                                color="#10b981" />
                <SummaryCard title="Faltante Total" value={formatCurrency(tableTotals.faltante)}                               color="#ef4444" />
                <SummaryCard title="Cumplimiento"   value={`${tableTotals.cumplimientoTotal}%`} color={getDynamicColor(tableTotals.cumplimientoTotal)} />
            </div>

            {/* ── TABLA REMOTA: DETALLE DE OPERACIÓN CALL CENTER ───────────── */}
            {jobId && apiClient && (
            ...
            {/* ── SUB-VISTAS (Llamadas / Mensajerías / Novedades) ──────────── */}
            <div className="space-y-6 w-full animate-in fade-in duration-500">
                {renderNavbar()}

                {/* PESTAÑA LLAMADAS — siempre montada, visibilidad por CSS */}
            ...
            {/* ── TABLA REMOTA: CALL CENTER – GESTIÓN Y NOVEDADES ─────────── */}
            {jobId && apiClient && (
